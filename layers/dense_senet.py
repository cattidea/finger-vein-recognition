...
class _ConvBlock(CustomLayer):
    ...
    def build(self, input_shape):
        num_ori_channels = input_shape[-1]
        num_new_channels = self.growth_rate
        ...
        self.att = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense((num_ori_channels + num_new_channels)//16),
            tf.keras.layers.Dense(num_new_channels, activation='sigmoid')
        ])
    ...
    def call(self, inputs):
        x = self.seq(inputs)
        att = self.att(self.concat([inputs, x]))
        return self.concat([inputs, self.multi([x, att])])
